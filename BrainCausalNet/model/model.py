...
class Embedding(BaseModel):
    ...
    def __init__(self, series_num, input_window, feature_dim, d_model, drop_prob, device):
        ...
        self.feature_emb = Linear(in_features=self.input_window * self.feature_dim, out_features=self.d_model,
                                  bias=True)
        # He Initialization
...
class CausalConv(BaseModel):
    ...
    def __init__(self, series_num, input_window, n_head, device):
        ...
        self.K = nn.Parameter(
            torch.ones((self.n_head, self.series_num, self.series_num, self.input_window), dtype=torch.float))
        self.register_parameter("kernel", self.K)
        self.mul = einsum('hxyji,bxif->bhxyjf')
        self.base = torch.tensor([i for i in range(1, self.input_window + 1)]).reshape(1, 1, 1, 1, -1, 1).to(
            self.device)
...
class MultiHeadAttention(BaseModel):
    ...
    def __init__(self, series_num, input_window, feature_dim, d_model, n_head, tau, device):
        ...
        self.attention = MultiVariateCausalAttention(self.series_num, self.input_window, self.feature_dim, self.d_model,
                                                     self.n_head, self.tau, self.device)
...
class Encoder(BaseModel):
    ...
    def __init__(self, series_num, input_window, feature_dim, d_model, n_head, n_layers, ffn_hidden, drop_prob, tau,
                 device):
        super().__init__()
...
class PredictModel(BaseModel):
    ...
    def relprop(self, rel):
        pad = torch.zeros((rel.shape[0], self.input_window - self.output_window, rel.shape[2], rel.shape[3])).to(
            self.device)
